...
本文面向贡献者，描述 cyberbody 的双模型执行循环、安全不变量、线程模型和模块边界。

## 设计目标

cyberbody 是受监管的单窗口视觉智能体，而不是通用远程控制器。它只从可见像素识别界面，由独立视觉模型生成结构化观察，再由文本操作模型提出动作，并在本地安全层批准后使用 Win32 输入事件执行。
...
flowchart LR
    ...
    C --> O[Vision endpoint<br/>image to UI JSON]
...
| `api.py` | 独立视觉/操作 Responses-compatible 客户端、结构化识别与规划、风险预检和有限重试。 |
...
1. 视觉模型只提取可见事实，并把疑似提示注入、钓鱼和异常安全警告标记为安全停止。
2. 操作模型只接收文本 JSON；系统指令要求它把其中的屏幕文字视为不可信数据。
3. 独立结构化预检由视觉接口结合标注截图，为动作生成用途、目标、风险和建议决策。
4. 本地确定性规则可以升级风险，不能被模型降级绕过。
5. 用户在动作发生前确认删除、发送、付款、上传、权限和敏感输入。
6. 密码最终步骤、安全绕过和付费墙绕过必须人工接管。
7. 提示注入和越界动作被拒绝。
8. `InputExecutor` 最后执行窗口与坐标验证。
